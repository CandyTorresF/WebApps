$(document).ready(function(){
    ...
    */
    ...
    // Acción para cargar las ºes previamente guardadas
    $.ajax({
    ...
    $.ajax({
        url : "data/comprobarRoot.php",
        type : "GET",
        dataType : "json",
        success : function(){
            $("#administrador").removeClass("hiddenRoot");
            
            // Ajax para el cargado de información en la vista del administrador
            $.ajax({
                url : "data/cargarTodosLosPedidos.php",
                type: "GET",
                dataType : "json",
                success : function(jsonReceived){
                    $.each(jsonReceived, function(index){    
                        $("#rootTable").append(
                            '<tr class="orden">' +
                            '<td>' + jsonReceived[index].usuario + '</td>' +
                            '<td class="idOrden">' + jsonReceived[index].numero + '</td>' +
                            '<td>' + jsonReceived[index].tipoHamburguesa + '</td>' +
                            '<td>' + jsonReceived[index].tipoPan + '</td>' +
                            '<td>' + jsonReceived[index].aderezos + '</td>' +
                            '<td>' + jsonReceived[index].cantidad + '</td>' +
                            '<td>' + jsonReceived[index].estado + '</td>' + 
                            '<td>' + '<input type="submit" class="cambiarEstado" value="Cambiar a terminado">' + '</td>'
                            + '</tr>'
                        );  
                ...
                error : function(){
                    alert("No se pudieron cargar todos los pedidos");
                }
            });
        },
        error : function(){
            alert("No se es el usuario root");
        }
    });
    
    // Script para cuando se le de click a un botó para cambiar el estado de una orden
    $(document).on('click', '.cambiarEstado', function () {
        var publicacion = $(this).closest('.orden');
        var identificador = $(publicacion).find('.idOrden').text();
        //alert("HOLA");
        //alert(identificador);
        ...
        //alert(jsonToSend.identificador);
        
        $.ajax({
            url: "data/cambiarEstadoPedido.php",
            type : "POST",
            data : jsonToSend,
            dataType : "json",
            success : function(jsonReceived){
